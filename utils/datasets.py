...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    # def __categorical_to_numerical(self, cat_classes):
    #     old = {}
    #     cont = 0
    #     num_classes = []
    #     for cl in cat_classes:
    #         if cl not in old.keys():
    #             old[cl] = cont
    #             cont += 1
    #         num_classes.append(old[cl])
    #     return num_classes
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, "r") as f:
                        ...
                        if any([len(x) > 8 for x in l]):  # is segment
                            # Changed to use categorical labels
                            classes = np.array([x[0] for x in l], dtype=np.float32)
                            # classes = np.array(self.__categorical_to_numerical([x[0] for x in l]), dtype=np.float32)
